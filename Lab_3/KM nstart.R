...
get_kmeans_nstart_results <- function(clrange, dfWithL, dfWithoutL){
  result <- c()
  ...
  for (m in c(5,10,20,50,100)){
    ...
    for (i in clrange){
      Kmeans <- kmeans(dfWithoutL, i, nstart = m)
...
get_plots_param <- function(result, PARAMrange, DFname, FOLDERname){
  ...
  legend("bottomleft", bg="transparent",c("nstart=5", "nstart=10", "nstart=20", "nstart=50", "nstart=100"), lwd=c(2,2,2,2,2), col=c("blue","orange","green","gray","red"), pch=c(15,15,15,15,15))
  title(paste("PAM", "\n", "PURITY", "\n", DFname, sep=""))
  dev.off()
  ...
  legend("topleft", bg="transparent",c("nstart=5", "nstart=10", "nstart=20", "nstart=50", "nstart=100"), lwd=c(2,2,2,2,2), col=c("blue","orange","green","gray","red"), pch=c(15,15,15,15,15))
  title(paste("PAM", "\n", "DUNN INDEX", "\n", DFname, sep=""))
  dev.off()
  ...
  legend("topright", bg="transparent",c("nstart=5", "nstart=10", "nstart=20", "nstart=50", "nstart=100"), lwd=c(2,2,2,2,2), col=c("blue","orange","green","gray","red"), pch=c(15,15,15,15,15))
  title(paste("PAM", "\n", "DB INDEX", "\n", DFname, sep=""))
  dev.off()
  ...
  result.df <-data.frame("NUM_of_clusters"=PARAMrange,
                         "PURITY_nstart=5"=result[[1]]@p, 
                         "PURITY_nstart=10"=result[[2]]@p,
                         "PURITY_nstart=20"=result[[3]]@p,
                         "PURITY_nstart=50"=result[[4]]@p,
                         "PURITY_nstart=100"=result[[5]]@p,
                         "DUNN_INDEX_nstart=5="=result[[1]]@d,
                         "DUNN_INDEX_nstart=10"=result[[2]]@d,
                         "DUNN_INDEX_nstart=20"=result[[3]]@d,
                         "DUNN_INDEX_nstart=50="=result[[4]]@d,
                         "DUNN_INDEX_nstart=100="=result[[5]]@d,
                         "DB_INDEX_nstart=5"=result[[1]]@db,
                         "DB_INDEX_nstart=10"=result[[2]]@db,
                         "DB_INDEX_nstart=20"=result[[3]]@db,
                         "DB_INDEX_nstart=50"=result[[4]]@db,
                         "DB_INDEX_nstart=100"=result[[5]]@db,
                         "AVG_SILHOUETTE_nstart=5"=result[[1]]@sil,
                         "AVG_SILHOUETTE_nstart=10"=result[[2]]@sil,
                         "AVG_SILHOUETTE_nstart=20"=result[[3]]@sil,
                         "AVG_SILHOUETTE_nstart=50"=result[[4]]@sil,
                         "AVG_SILHOUETTE_nstart=100"=result[[5]]@sil)
...
f_km_metric <- function(){
  ...
  result.km.method <- get_kmeans_nstart_results(clrange, df, df.dataonly)
  result.km.method.stand <- get_kmeans_nstart_results(clrange, df.stand, df.stand.dataonly)
  
  get_plots_param(result.km.method, clrange, "wine default", "KM WINE nstart")
  get_plots_param(result.km.method.stand, clrange, "wine standardized", "KM WINE STAND nstart")
}
